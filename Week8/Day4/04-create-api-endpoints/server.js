...
const server = http.createServer((req, res) => {
  ...
  req.on("end", () => {
    ...
    /* ======================== ROUTE HANDLERS ======================== */
    // 200 <- get/put/patch/delete, 201 <- post
    ...
    if (req.method === "GET" && req.url.startsWith("/dogs/")) {
      ...
      if (urlParts.length === 3) {
        ...
        // Your code here
        specificDog = dogs.find((dog) => dog.dogId == dogId);
      }

      res.statusCode = 200;
      res.setHeader("Content-Type", "application/json");
      return res.end(JSON.stringify(specificDog));
    }
    ...
    if (
      ...
      if (urlParts.length === 3) {
        const dogId = urlParts[2];
        // Your code here
        foundDog = dogs.find((dog) => dog.dogId === +dogId);
        if (foundDog) {
          const { name, age } = req.body;
    ...
    if (req.method === "DELETE" && req.url.startsWith("/dogs/")) {
      ...
      if (urlParts.length === 3) {
        const dogId = urlParts[2];
        // Your code here
        const foundDogIndex = dogs.findIndex(
          (dog) => dog.dogId === Number(dogId)
        );
